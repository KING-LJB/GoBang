...
//算法核心类，算法的主体思想分三个步骤，
//第一步：根据双方的当前的形势  循环地假设性的  分别给自己和对方下一子（在某个范围内下子），并判断此棋子能带来的形势上的变化，如能不能冲4，能不能形成我方或敌方双3等，
...
public class BaseComputerAi extends BasePlayer {
    ...
    /**
     * 初始化棋盘范围
     * @param comuters : 电脑所下的棋子集合
     * @param humans : 人类玩家所下的棋子集合
     */
...
//  private int debugx,debugy;//用于DEBUG
    ...
    private Point doFirstAnalysis(List<Point> comuters, List<Point> humans) {
        ...
        /**
         * 如下是对一个位置进行分析：
         * 我方下和对方下的两种不同情况下，在各个方向上所产生的不同结果
         */
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (x < currentRange.xStart || x > currentRange.xStop || y < currentRange.yStart || y > currentRange.yStop) {
                continue; // continue就是中断本次循环开始下一次。
            }
...
//          }
            ...
            /**
             * 尝试在此位置上下一个棋子，并分析该棋子对 我方（电脑） 可形成的状态，如活4，活3，半活4，活2等所有状态
             */
            //横向
            ...
            if (firstAnalysisResult != null) {//无返回结果此方向上不可能达到五个棋子，
                ...
                //记录第一次分析结果,该方法是把第一次的分析结果加入到对应玩家（人类和电脑）的第一次分析结果集中
            ...
            /**
             * 尝试在此位置上下一个棋子，并分析该棋子对敌方（人类）可形成的状态，如活4，活3，半活4，活2等所有状态
             */
            //横向
    ...
    private Point doComputerSencondAnalysis(Map<Point, List<FirstAnalysisResult>> firstResults, List<SencondAnalysisResult> sencodResults) {
        ...
        for (Point p : firstResults.keySet()) {
            ...
            // 把遍历选择第一次分析结果后产生最好的结果放到第二次分析结果集中
    ...
    /**
     * 电脑用自己在第二次筛选时产生的最好的一步棋与敌方的第二次分析结果集进行比较
     *如果自己最好的一步棋的集合中包含敌方最好的一步棋，那么就下这步棋
     * 如果不包含，就下自己最好的一步棋
     *
     * @param myBest : 自己最好的一步棋的集合
     * @param yourSencodResults : 敌方的第二次结果集
     * @return
     */
    ...
    private class FirstAnalysisResult {
        ...
        private FirstAnalysisResult init(Point point, int direction, int aliveState) {
            ...
            this.point = point;
            this.direction = direction;
            this.aliveState = aliveState;// ALIZE=1 , HALF_ ALIZE=0
            return this;
        }
    ...
    //返回-1则第一个参数优先，1则第二个参数优先，0则按原来顺序
    ...
    // 分析如果在当前位下一子，会形成某个方向上多少个子，参数：当前己方已下的所有点，当前敌方所下的所有点，当前要假设的点，需要判断的方向
    private FirstAnalysisResult tryAndCountResult(List<Point> myPoints, List<Point> enemyPoints, Point point, int direction) {
        ...
        // maxCountOnThisDirection:表示在某个方向上是否还空间能在下5个棋子
